function [A0,A1,A2,Ap,Alag,Blag,Clag] = rational_approx(genAIC,b_poles,omega)
...
if isempty(idx_omege0)      % If all omegas are nonzero
    A = [ones(NumOmega,1) zeros(NumOmega,1) -omega'.^2 numr./denm;
         zeros(NumOmega,1) omega' zeros(NumOmega,1) numi./denm];
else    % If one omega is zero
    a0 = genAIC(:,:,idx_omege0);
    if ~isreal(a0)
        error('Expecting real values of AIC for omega=0') 
    end
    A = [zeros(NumOmega,1) -omega'.^2 numr./denm;
           omega' zeros(NumOmega,1) numi./denm];
end


% Obtain b and solve X = A\b
X = zeros([size(genAIC,1),size(genAIC,2),NumPoles+3]);

for irow = 1:size(genAIC,1)
    for icol = 1:size(genAIC,2)
        
        % Define a diagonal scaling factor to improve conditioning
        diag_scale =  diag(([1./abs(squeeze(genAIC(irow,icol,:)));...
            1./abs(squeeze(genAIC(irow,icol,:)))]));
        ...
        if isempty(idx_omege0)
            b_scale = diag_scale*[real(squeeze(genAIC(irow,icol,:)));...
                imag(squeeze(genAIC(irow,icol,:)))];
...
for ii = 1:NumPoles
    Alag = blkdiag(Alag,-b_poles(ii)*eye(size(genAIC,2)));
    Blag = [Blag; eye(size(genAIC,2))];
    Clag = [Clag Ap(:,:,ii)];
end
