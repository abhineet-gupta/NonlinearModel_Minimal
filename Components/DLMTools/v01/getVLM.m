function [Dfinal,AIC] = getVLM(PC,PA,PB,PAreas,Pspans,n_hat_w,n_hat_wl)
% Inputs:
%  PC := 3-by-NumPanels matrix of the collocation points. The i^th column
%       of PC gives the (x,y,z) coordinations of the i^th collocation
%       point.
%
%  PA,PB := 3-by-NumPanels matrices containing the points describing the
%       horseshoe vortices for each panel. The i^th column of each matrix
%       defines the (x,y,z) coordinates of the i^th point. The i^th 
%       horseshoe vortex consists of:
...
%  PAreas := 1-by-NumPanels vector of the panel areas. The unit is meters
%       square
%  n_hat_w := 1-by-NumPanels binary vector. Contains 1 if the panel belongs
%           to the planform (wing + centerbody), 0 otherwise.
%  n_hat_wl := 1-by-NumPanels binary vector. Contains 1 if the panel 
%           belongs to the winglets, 0 otherwise
%
% Output:
%  Dfinal := NumPanels-by-NumPanels matrix which when multiplied by the
%           vector Cp/Vinf (Cp is the pressure coefficient of each panel),
%           outputs the normalwash at each panel.
%  AIC := NumPanels-by-NumPanels matrix which when multiplied by the
%           normalwash vector, outputs the Cp/Vinf vector
%
%
% The mathematical derivation of Dfinal & AIC is described here
% Del_P = rho*vinf*Gamma*span
% 0.5*rho*vinf^2*area*Cp = rho*vinf*Gamma*span
% 0.5*rho*vinf^2*area*Cp = rho*vinf*span*(D\normalwash)
% Cp = span/(0.5*vinf*area) * (D\normalwash)
% Cp = ((0.5*area/span) * D )^-1 * (normalwash/vinf);
% Cp = Dfinal^-1 * (normalwash/vinf);
% Cp = AIC * (normalwash/vinf);
% Dfinal * Cp = (normalwash/vinf)
%
% Reference: 
...
%% Get normal vectors
% n_hat_w indicates the cosine of panel dihedrals (1 for wing, 
%     0 for winglet panels)
% n_hat_wl indicates the sine of panel dihedrals 
n_hat_w = repmat(n_hat_w', [1 NumPanels] ); 
n_hat_wl = repmat(n_hat_wl', [1 NumPanels] ); 
...
%% Compute Panel Spans
F = 0.5*PAreas./Pspans;
F = repmat(F,[NumPanels 1]);
Dfinal = D.*F;   

%% Output AIC, if required
if nargout == 2
    AIC = inv(Dfinal);
end