...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	% Create gs command.
    % XXX PJS: dUseCIEColor=true causes Matlab plots to have a cream 
    % background. I modified the option to be false and this gives white
    % backgrounds.  I also removed the UseDeviceIndependentColor option
    % since this causes Ghostscript warnings when dUseCIEColor=false
    ...
    epsCmd=	[epsCmd ' -dUseCIEColor=false'];
...
%	device.png.cmyk=	sprintf(cmdEnd,	'png???', 					[fname '.png']);
	...
	switch(computer)													% Get gs command.
		...
		case {'PCWIN','PCWIN64'},		
            gs = '"C:\Program Files\gs\gs9.07\bin\gswin64c.exe"';    
            %gs= 'gswin32c.exe';
		otherwise,						gs= 'gs';
	end
